...
export class CodepipelineStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    /* existing bucket */
    const pipelineArtifactBucket = s3.Bucket.fromBucketName(
      this,
    ...
    const topic = sns.Topic.fromTopicArn(
      this,
      "mysnstopic",
      this.node.tryGetContext("topicARN")
    );
    ...
    /* existing IAM role */
    const codeBuildExecutionRole = iam.Role.fromRoleArn(
      this,
      "codebuildiamrole",
      this.node.tryGetContext("codebuildroleARN")
    );

    /* Build  */
    const code_build_project = new codebuild.PipelineProject(
      this,
      "CodeBuild",
      {
    ...
    /* Unit Test  */
    const code_build_unit_test_project = new codebuild.PipelineProject(
      this,
      "UnitTest",
      {
    ...
    /* existing IAM role */
    const pipelineExecutionRole = iam.Role.fromRoleArn(
      this,
      "pipelineiamrole",
      this.node.tryGetContext("pipelineroleARN")
    );
          ...
          actions: [
            new codepipeline_actions.S3DeployAction({
              ...
              bucket: s3.Bucket.fromBucketName(
                this,
                "DeployBucket",
                this.node.tryGetContext("deploymentbucketName")
              ),
              objectKey: "cicd-pipeline",
            }),
          ],
        },
        {
